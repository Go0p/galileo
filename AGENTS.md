...
- **无缓存原则**：对远端 Quote API 不做缓存；如需缓存其他数据，需阐明过期策略与命中率。
...
- Quote / Swap API 行为尽量在本地沙箱或回放环境验证；上线前确保 `galileo strategy` 在 dry-run 下运行成功。
...
- 与第三方依赖（Jito、DFlow、Ultra、Kamino 等）交互方式变化时，立即同步团队并标注兼容性风险。
