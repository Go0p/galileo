...
- 快照格式含 `version`、`generated_at` 与 `entries`，便于兼容升级；写文件采用 “临时文件 + 原子替换” 防止半写。
...
- 允许记录来源（snapshot/observation），以便调试与后续统计。
