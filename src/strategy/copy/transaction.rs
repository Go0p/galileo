...
use crate::instructions::jupiter::parser::PROGRAM_ID as PARSER_PROGRAM_ID;
use crate::instructions::jupiter::parser::{RouteKind, RouteV2Accounts};
...
impl RouteContext {
    pub fn from_instruction(instruction: &Instruction) -> Option<Self> {
        ...
        match crate::instructions::jupiter::parser::classify(&instruction.data) {
...
pub(crate) fn derive_associated_token_address(
    ...
    Ok(cached_associated_token_address(owner, mint, token_program))
}
...
pub(crate) fn split_compute_budget(
...
) -> (Vec<Instruction>, Vec<Instruction>, Option<u64>) {
    ...
    if let Some(price) = price_mode.and_then(sample_compute_unit_price) {
        if price > 0 {
            ...
            sampled_price = Some(price);
        }
    }

    (compute_budget, main, sampled_price)
}
...
mod tests {
    ...
    fn route_v2_data(in_amount: u64, quoted_out: u64, slippage: u16) -> Vec<u8> {
        let mut data = crate::instructions::jupiter::parser::discriminator::ROUTE_V2.to_vec();
    ...
    fn shared_route_v2_data(id: u8, in_amount: u64, quoted_out: u64, slippage: u16) -> Vec<u8> {
        let mut data =
            crate::instructions::jupiter::parser::discriminator::SHARED_ROUTE_V2.to_vec();
        data.push(id);
        data.extend_from_slice(&in_amount.to_le_bytes());
        data.extend_from_slice(&quoted_out.to_le_bytes());
        data.extend_from_slice(&slippage.to_le_bytes());
    ...
    fn route_legacy_data(in_amount: u64, quoted_out: u64, slippage: u16) -> Vec<u8> {
        let mut data = crate::instructions::jupiter::parser::discriminator::ROUTE.to_vec();
