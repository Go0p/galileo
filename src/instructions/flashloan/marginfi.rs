...
use super::types::{FlashloanMetadata, FlashloanOutcome, FlashloanProtocol};
use crate::engine::plugins::flashloan::FlashloanError;
use crate::engine::plugins::flashloan::marginfi::{
    ACCOUNT_INITIALIZE_DISCRIMINATOR, BEGIN_DISCRIMINATOR, BORROW_DISCRIMINATOR,
    CLOSE_ACCOUNT_DISCRIMINATOR, END_DISCRIMINATOR, GROUP_ID, PROGRAM_ID, REPAY_DISCRIMINATOR,
    SYSTEM_PROGRAM_ID, TOKEN_PROGRAM_ID, compute_associated_token_address, parse_pubkey,
};
use crate::instructions::flashloan::FlashloanResult;
...
impl MarginfiFlashloan {
    ...
    pub fn wrap(
        ...
        let destination = compute_associated_token_address(identity_pubkey, &asset.mint);
        ...
        let begin_ix = build_begin(self.account, *identity_pubkey, end_index);
...
pub fn build_initialize_instruction(
    ...
    let mut data = Vec::with_capacity(ACCOUNT_INITIALIZE_DISCRIMINATOR.len());
    data.extend_from_slice(&ACCOUNT_INITIALIZE_DISCRIMINATOR);

    let accounts = vec![
        AccountMeta::new(*GROUP_ID, false),
...
pub fn build_close_instruction(
    ...
    Ok(Instruction {
        ...
        data: CLOSE_ACCOUNT_DISCRIMINATOR.to_vec(),
    })
}
